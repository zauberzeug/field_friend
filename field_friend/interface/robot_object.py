...
class robot_object(robot_object):
        ...
        with self:
            ...
            if isinstance(self.robot.y_axis, YAxis):
                ...
                with Group() as self.second_tool:
                    Box(0.03, 0.03, 0.35).material('#4488ff').move(z=0.3)
                    Cylinder(0.05, 0, 0.05).material('#4488ff').move(z=0.1).rotate(np.pi/2, 0, 0)
    ...
    def update(self) -> None:
        ...
        if isinstance(self.robot.y_axis, YAxis):
            ...
            self.second_tool.move(x=self.robot.WORK_X_DRILL, y=self.robot.y_axis.position, z=-self.robot.z_axis.depth)
