    ...
    with ui.card():
        if isinstance(field_friend, rosys.hardware.RobotHardware):
            ...
            with ui.card():
                ...
                ui.button(text='set IMU', on_click=lambda: field_friend.imu.simulate_measurement(
                    rosys.geometry.Rotation.from_euler(roll=np.radians(r.value), pitch=np.radians(p.value), yaw=np.radians(y.value))))
