        ...
        for path in self.path_provider.paths:
            with ui.card().classes('items-stretch'):
                ...
                with ui.row().classes('items-canter'):
                    ...
                    with ui.row().bind_visibility_from(self.path_recorder, 'current_path_recording', lambda p, name=path.name: p == name):
                        ui.button('recording...', on_click=self.stop_recording) \
                            .props('icon=radio_button_checked color=red fab-mini flat') \
                    ...
                    with ui.row().bind_visibility_from(path, 'path_segments', lambda path_segments: path_segments != []):
                        ui.button('visualize', on_click=lambda _, path=path: self.path_provider.SHOW_PATH.emit(path.path_segments)) \
                            .props('icon=visibility fab-mini flat') \
                            .tooltip('visualize path')
                        ui.button('drive', on_click=lambda path=path: self.automator.start(self.path_recorder.drive_path(path))) \
                            .props('icon=play_arrow fab-mini flat').tooltip('drive recorded path') \
                    ...
                    with ui.row().bind_visibility_from(self.path_recorder, 'current_path_driving', lambda p, name=path.name: p == name):
                        ui.button('stop', on_click=self.stop_driving) \
                            .props('icon=stop fab-mini flat').tooltip('stop driving') \
