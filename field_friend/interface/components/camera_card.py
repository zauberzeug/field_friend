...
class camera_card:
    ...
    def __init__(self, system: 'System', shrink_factor: float = 4.0) -> None:
        ...
        for point in detections.points:
            if point.category_name in self.plant_locator.crop_category_names:
                svg += f'''<circle cx="{int(point.x / self.shrink_factor)}" cy="{int(point.y / self.shrink_factor)}" \
                    r="18" stroke-width="8" stroke="green" fill="none"/>'''
            elif point.category_name in self.plant_locator.weed_category_names:
                svg += f'''<line x1="{int(point.x / self.shrink_factor) - cross_size}" y1="{int(point.y / self.shrink_factor)}" \
                        x2="{int(point.x / self.shrink_factor) + cross_size}" y2="{int(point.y / self.shrink_factor)}" \
                        stroke="red" stroke-width="8" transform="rotate(45, {int(point.x / self.shrink_factor)}, \
                        {int(point.y / self.shrink_factor)})"/>\
                        <line x1="{int(point.x / self.shrink_factor)}" y1="{int(point.y / self.shrink_factor) - cross_size}" \
                        x2="{int(point.x / self.shrink_factor)}" y2="{int(point.y / self.shrink_factor) + cross_size}" \
    ...
    def build_svg_for_implement(self) -> str:
        ...
        svg += f'<line x1="{int(min_tool_2d.x)}" y1="{int(min_tool_2d.y)}" x2="{int(max_tool_2d.x)} \
            " y2="{int(max_tool_2d.y)}" stroke="black" stroke-width="2" />'
        if self.show_weeds_to_handle:
            for i, plant in enumerate(self.system.current_implement.weeds_to_handle.values()):
                ...
                if screen is not None:
                    svg += f'''<circle cx="{int(screen.x/self.shrink_factor)}" cy="{int(screen.y / self.shrink_factor)}" \
                        r="6" stroke="blue" fill="transparent" stroke-width="2" />'''
                    svg += f'''<text x="{int(screen.x/self.shrink_factor)}" y="{int(screen.y / self.shrink_factor)+4}"\
    ...
    def build_svg_for_plant_provider(self) -> str:
        ...
        for plant in self.plant_provider.get_relevant_weeds(position):
            ...
            if screen is not None:
                svg += f'''<circle cx="{int(screen.x/self.shrink_factor)}" cy="{int(screen.y / self.shrink_factor)}"\
                    r="5" fill="white" />'''
                svg += f'''<text x="{int(screen.x/self.shrink_factor)}" y="{int(screen.y/self.shrink_factor) + 16}"\
                    fill="black" font-size="9" text-anchor="middle">{plant.id[:4]}</text>'''
        return svg
