...
class ZAxisHardwareV2(ZAxisV2, rosys.hardware.ModuleHardware):
    ...
    async def move_to(self, depth: float, speed: float = ZAxisV2.MAX_SPEED) -> None:
        try:
...
class ZAxisSimulationV2(ZAxisV2, rosys.hardware.ModuleSimulation):
    ...
    async def move_to(self, depth: float, speed: int = ZAxisV2.MAX_SPEED) -> None:
        try:
            await super().move_to(depth, speed)
        except RuntimeError as e:
            rosys.notify(e, type='negative')
