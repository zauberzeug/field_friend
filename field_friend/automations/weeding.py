...
class Weeding(rosys.persistence.PersistentModule):
            ...
            if self.crops_to_handle:
                ...
                if weeds_in_range:
                    ...
                    if axis_distance >= 0:
                        ...
                        self.log.warning(f'Crop position {next_weed_position} is behind field friend')
                if not moved:
                    await self._follow_line_of_crops()
                    moved = True
            if not moved:
    ...
    async def _create_simulated_plants(self):
        ...
        if self.current_segment:
            ...
            distance = self.current_segment.spline.start.distance(self.current_segment.spline.end)/10
