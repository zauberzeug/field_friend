...
class Operation:
    ...
    def field_setting(self) -> None:
        with ui.dialog() as self.edit_field_dialog, ui.card():
            parameters: dict = {
                'name': self.field_provider.selected_field.source.name if self.field_provider.selected_field else '',
                'row_count': self.field_provider.selected_field.source.row_count if self.field_provider.selected_field else 0,
                'row_spacing': self.field_provider.selected_field.source.row_spacing if self.field_provider.selected_field else 0.0,
                'outline_buffer_width': self.field_provider.selected_field.source.outline_buffer_width if self.field_provider.selected_field else 2.0,
                'bed_count': self.field_provider.selected_field.source.bed_count if self.field_provider.selected_field else 1,
                'bed_spacing': self.field_provider.selected_field.source.bed_spacing if self.field_provider.selected_field else 0.5,
                'bed_crops': self.field_provider.selected_field.source.bed_crops if self.field_provider.selected_field else {'0': None}
            }
                ...
                if self.field_provider.selected_field.source.bed_count > 1:
                    with ui.row().classes('w-full'):
                        ...
                        with ui.row().bind_visibility_from(beds_checkbox, 'value').classes('w-full'):
                            ui.select(list(range(0, int(self.field_provider.selected_field.source.bed_count))),
