...
assert_equal(odrv0.axis1.config.can.node_id, 0x600 >> 5)
...
for i, axis in enumerate([odrv0.axis1]):
    ...
    assert_equal(axis.motor.config.pole_pairs, 2)
    assert_equal(axis.motor.config.calibration_current, 2)
    assert_equal(axis.motor.config.current_lim, 7)
    assert_equal(axis.motor.config.current_lim_margin, 5)
    assert_equal(axis.motor.config.torque_constant, 1.0)
    assert_equal(axis.motor.config.torque_lim, 5)
    ...
    assert_equal(axis.encoder.config.cpr, 12)
    ...
    assert_equal(axis.controller.config.vel_differentiator_gain, 0.0)
    assert_equal(axis.controller.config.vel_limit, 60)
    assert_equal(axis.controller.config.pos_gain, 20.0)
