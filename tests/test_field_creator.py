...
def test_field_creation(system: System, field_creator: FieldCreator):
    ...
    assert system.field_provider.fields[0].bed_count == 2
    assert system.field_provider.fields[0].bed_spacing == 1.0
    assert system.field_provider.fields[0].bed_crops == {'0': 'sugar_beet', '1': 'garlic'}
...
def test_support_point_dialog(system: System, field: Field):
    ...
    dialog.next()
    dialog.next()
    row_support_point_cartesian = system.field_provider.fields[0].row_support_points[0].to_local()
