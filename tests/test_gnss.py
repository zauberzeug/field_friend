...
async def test_driving(gnss_driving: System):
    assert gnss_driving.robot_locator.pose.point.x == 0
    assert gnss_driving.robot_locator.pose.point.y == 0
...
async def test_record_is_none(gnss_driving: System, gnss: GnssSimulation):
    ...
    await forward(5)
    # robot should have stopped driving
    assert_point(gnss_driving.robot_locator.pose.point, rosys.geometry.Point(x=2, y=0))
